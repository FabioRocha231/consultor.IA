...
export function StatusPill({ variant = "connected", children }) {
  const variants = {
    connected: "bg-green-500/15 text-green-500 light:text-green-700",
    disconnected: "bg-zinc-500/15 text-zinc-400 light:text-slate-600",
    error: "bg-red-500/15 text-red-500 light:text-red-700",
  };

  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${
        variants[variant] || variants.connected
      }`}
    >
      {children}
    </span>

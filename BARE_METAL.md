# Run consultor.IA in production without Docker

> [!WARNING]
> This method of deployment is **deprecated** and **not supported** for consultor.IA. It is kept as a reference only.
> The recommended path is the Docker image in `docker/`.
> You are fully responsible for securing your deployment and data in this mode.

Here you can find the scripts and known working process to run consultor.IA outside of a Docker container.
...
2. `cd consultor.IA` and run `yarn setup`. This will install all dependencies to run in production as well as debug the application.
...
DB_URL="postgresql://consultor:consultor@localhost:5432/consultor"
```
...
consultor.IA is comprised of three main sections. The `frontend`, `server`, and `collector`. When running in production you will be running `server` and `collector` on two different processes, with a build step for compilation of the frontend.
...
3. Migrate and prepare your database.
```
...
consultor.IA should now be running on `http://localhost:3001`!

## Updating consultor.IA

To update consultor.IA with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple consultor.IA processes on the same instance as this can cause conflicts.
...
cd $HOME/consultor.IA/frontend && yarn && yarn build && cd $HOME/consultor.IA
...
cd $HOME/consultor.IA/collector && yarn

echo "Installing server dependencies & running migrations"
cd $HOME/consultor.IA/server && yarn
cd $HOME/consultor.IA/server && npx prisma migrate deploy --schema=./prisma/schema.prisma
cd $HOME/consultor.IA/server && npx prisma generate

echo "Booting up services."
truncate -s 0 /logs/server.log # Or any other log file location.
truncate -s 0 /logs/collector.log

cd $HOME/consultor.IA/server
(NODE_ENV=production node index.js) &> /logs/server.log &

cd $HOME/consultor.IA/collector

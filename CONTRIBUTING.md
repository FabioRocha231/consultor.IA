# Contributing to consultor.IA

consultor.IA is an open-source project and we welcome contributions from the community.

## Reporting Issues

If you encounter a bug or have a feature request, please open an issue on the
[GitHub issue tracker](https://github.com/FabioRocha231/consultor.IA/issues).

## Picking an issue

We track issues on the GitHub issue tracker. If you are looking for something to work on, check the [good first issue](https://github.com/FabioRocha231/consultor.IA/contribute) label. These issues are typically the best described and have the smallest scope. There may be issues that are not labeled as good first issue, but are still a good starting point.
...
1. If you are modifying the permission system for a new role or something custom, you are likely better off building your own version since this is a core part of the project and is only to be maintained by the consultor.IA team.
...
4. **Security** is always important. If you have a security concern, please do not open an issue. Instead, follow the reporting process in [SECURITY.md](./SECURITY.md).
...
Then confirm your fork is set as `origin`:

```bash
git remote -v
```

For upstream synchronization, follow the strategy documented in the README's `Upstream Strategy` section.
...
1. If you change the public API, update the repository documentation.
1. Aim to minimize the number of changes in each pull request. Keep to solving one problem at a time, when possible.
1. Before marking a pull request ready-for-review, do a self review of your code. Is it clear why you are making the changes? Are the changes easy to understand?
...
1. If your pull request is a work in progress, leave the pull request as a draft. We will assume the pull request is ready for review when it is opened.
1. When writing tests, test the error cases. Make sure they have understandable error messages.

## Project structure

The core library is written in Node.js. The embed widget and browser extension are vendored in this monorepo and are no longer separate git submodules.

* `server`: Node.js server source code
* `frontend`: React frontend source code
* `collector`: Node.js collector source code
* `embed`: vendored web embed widget
* `browser-extension`: vendored browser extension
* `docs`: architecture, ADRs and product documentation
* `docker`: Docker image and compose files

## Release process

Changes to consultor.IA are integrated through the `master` branch. Merged PRs should keep the Docker build green using `docker/Dockerfile`. This fork does not currently publish automatic public release artifacts; deployments are managed by each operator from this repository.

## Criteria for New LLM Providers

To ensure the long-term maintainability of consultor.IA and prevent repository bloat, we enforce a vetting process for adding new third-party LLM provider integrations.
...
> These guidelines apply strictly to unsolicited community or third-party startup contributions. If you are an ecosystem, silicon, or cloud hardware partner engaging directly with the upstream core team on a co-developed integration, proof-of-concept, or native optimization project, this vetting process is not applicable.
...
We cannot act as a discovery or marketing engine for early-stage startups. To qualify for codebase inclusion, a provider must demonstrate an active, existing user base who would benefit from consultor.IA's functionality. Any of the following criteria are acceptable:
...
We are an AI company and we expect contributors to use AI tools. However, we believe in **AI-augmented engineers**, not AI-replaced engineers. There is a difference between using an LLM to help you write code and having an LLM write code for you.
...
By contributing to consultor.IA (this repository), you agree to license your contributions under the MIT license.

consultor.IA is based on AnythingLLM, originally developed by Mintplex Labs, under the MIT License. The original copyright and license text are preserved in [LICENSE](./LICENSE).
